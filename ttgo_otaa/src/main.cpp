...
static const u1_t PROGMEM DEVEUI[8]= { 0x59, 0x7F, 0x30, 0xF8, 0x32, 0x7D, 0xC3, 0x00 };
void os_getDevEui (u1_t* buf) { memcpy_P(buf, DEVEUI, 8);}

// Copy the value from Application EUI from the TTN console in LSB mode
static const u1_t PROGMEM APPEUI[8]= { 0x57, 0xAD, 0x01, 0xD0, 0x7E, 0xD5, 0xB3, 0x70 };
void os_getArtEui (u1_t* buf) { memcpy_P(buf, APPEUI, 8);}

// This key should be in big endian format (or, since it is not really a
// number but a block of memory, endianness does not really apply). In
// practice, a key taken from ttnctl can be copied as-is. Anyway its in MSB mode.
static const u1_t PROGMEM APPKEY[16] = { 0x72, 0x8E, 0x8F, 0xA0, 0xDA, 0xEF, 0xFF, 0xD4, 0x49, 0x06, 0x15, 0xAD, 0x88, 0x4B, 0x10, 0xA7 };
...
void do_send(osjob_t* j){
    ...
    static uint8_t message[] = {0,y,99};

    // Check if there is not a current TX/RX job running
    if (LMIC.opmode & OP_TXRXPEND) {
        Serial.println(F("OP_TXRXPEND, not sending"));
        os_setTimedCallback(&sendjob, os_getTime()+sec2osticks(TX_INTERVAL), do_send);
        Serial.print(F("next scheduled.... "));
    } else {
        // Prepare upstream data transmission at the next possible time.

        LMIC_setTxData2(1, message, sizeof(message)-1, 0);
        Serial.println(F("preparing uplink packet..."));
...
void onEvent (ev_t ev) {
    ...
    switch(ev) {
           case EV_TXCOMPLETE:
            ...
            break;
        case EV_JOINING:
            Serial.println(F("EV_JOINING: -> Joining2.."));
        ...
        case EV_JOINED: {
              ...
              display.clear();
              display.drawString(0 , 0 ,  "Joined 2!");
              display.display();
              // Disable link check validation (automatically enabled
              // during join, but not supported by TTN at this time).
              LMIC_setLinkCheckMode(0);
              os_setTimedCallback(&sendjob, os_getTime()+sec2osticks(TX_INTERVAL), do_send);
              Serial.print(F("next scheduled2... "));
            }
            break;
        case EV_RXCOMPLETE:
